...
var prevpause = pause
if (pause && (!instance_exists(obj_option)) && (!instance_exists(obj_lvlselect)) && alarm[3] == -1)
{
    scr_getinput()
    ...
    selected += moveselect
    if (moveselect != 0 && selected >= 0 && selected <= 4)
    {
    ...
    if key_jump
    {
        switch selected
        {
            case 0:
                ...
                instance_destroy(obj_lvlselect)
            ...
            case 2:
                fmod_event_one_shot("event:/sfx/ui/select")
                with (instance_create(x, y, obj_lvlselect))
                    depth = (other.depth - 1)
                break
            case 4:
