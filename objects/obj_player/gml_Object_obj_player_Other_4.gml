...
{
	obj_player.backtohubstartx = 0
	obj_player.backtohubstarty = 0
}
...
if(global.levelselect)
{
	obj_player.backtohubstartx = levelselect_realhubx(global.leveltosave)
	obj_player.backtohubstarty = levelselect_realhuby(global.leveltosave)
	var lv = levelselect_dest_level(global.ls_last_floor, global.ls_last_level)
	var dest_type = levelselect_dest_desttype(global.ls_last_floor, global.ls_last_level, global.ls_last_section)
	switch(dest_type)
	{
		case "level":
			if(global.ls_last_section == 1 && lv != "war")
				levelselect_prep_john(lv)
			break
		case "secret-lv":
			levelselect_prep_levelsecret(lv, global.ls_last_section)
			break
		case "secret-fl":
			levelselect_prep_floorsecret(lv)
			break
		case "boss":
			break
	}
